...
void R_SetupFrame (AActor *actor)
{
	...
	//VIZDOOM_CODE: observer-only viewpoint; actor, player and interpolation state stay intact.
	if (doomflyObserverActive)
	{
		viewx = fixed_t(doomflyObserverPose[0] * FRACUNIT);
		viewy = fixed_t(doomflyObserverPose[1] * FRACUNIT);
		viewz = fixed_t(doomflyObserverPose[2] * FRACUNIT);
		viewangle = angle_t(doomflyObserverPose[3] * (4294967296.0 / 360.0));
		viewpitch = int(doomflyObserverPose[4] * (4294967296.0 / 360.0));
		viewsector = R_PointInSubsector(viewx, viewy)->sector;
		r_showviewer = false;
	}
